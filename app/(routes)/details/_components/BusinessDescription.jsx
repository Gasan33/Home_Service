...
const BusinessDescription = ({ business }) => {
    return business.name && (
        <div>
            ...
            <div className='grid grid-col-2 md:grid-cols-3 lg:grid-cols-4 gap-5 mt-5'>
                {business.images.map((item, index) => (
                    <Image key={index} src={item.url} alt='image' width={700} height={200} className='rounded-lg' />
                ))}
            </div>
        </div>
    )
